...
### monitor使用方法（非必须，可跳过）
主要参考https://github.com/HighTorque-Robotics/RoboCup_Monitor中Readme部分，强调一下teamID部分，如下图：
...
### 常见Q&A汇总
#### 1、执行roslaunch dlaunch pi_plus.launch指令，发现机器人头部无法正常扫描
答：打开core/src/dlaunch/launch/路径下的piplus.launch文件，将文件中这一行选中后同时按下“ctrl+/”进行注释
![17](/tutorial-images/image(22).png)
然后在终端中并行启动下方两个launch文件，即可恢复视觉追球效果

```bash
cd RoboCup_Workspace/core
source devel/setup.bash
roslaunch dlaunch piplus.launch
rolaunch dbehavior default.launch
```


#### 2、机器人无法连接自己的手机热点或自备的路由器
答：因机器人的无线网卡驱动只暂时支持2.4GHZ频段连接，如果是自身手机热点，需将AP频段设置为2.4GHZ;如果是自己携带的路由器需确保支持2.4/5GHZ频段。
...
#### 8、如果无法克隆RoboCup_Workspace仓库，从本地把RoboCup_Workspace文件夹拖拽到机器人上，在运行roslaunch dlaunch pi_plus.launch出现main.py文件没有权限
答：在终端依次执行
```bash
cd RoboCup_Workspace/core/src/dbehavior/src
sudo chmod 777 main.py
```
再次运行roslaunch dlaunch pi_plus.launch即可正常拉起视觉程序
